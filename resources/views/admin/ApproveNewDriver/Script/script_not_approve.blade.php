<script>
    $('body').on('click', '.btn-not-approve', function(){
        var id = $(this).data('id');
        var btn = $(this);
        $('#add_not_approve_driver_interview_id').val(id);
        $("#ModalNotApprove").modal('show');
    });

    $('body').on('submit', '#FormNotApprove', function(e) {
        e.preventDefault();
        var form = $(this);
        var id = $('#add_not_approve_driver_interview_id').val();
