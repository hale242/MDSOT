<div class="modal-header">
    ...
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><iclass="mdi mdi-close"></i></span></button>
    @php
        // GF::print_r(count($drivermatch));
    @endphp
</div>
                            ...
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                       ...
                                       <th scope="col" class="text-left">Firstname - Lastname</th>
                                ...
                                <tbody>
                                    @php
                                        // GF::print_r(explode(',',$filters[0]['gender']));
                                        // GF::print_r($drivermatch);
                                        // GF::print_r($drivermatch[3]['DriverLanguage'][0]['language_type_id']);
                                        // GF::print_r($filters[0]);
                                        $totalfilter = 0;
                                        ...
                                        if($filters[0]['age_start']!=''||$filters[0]['age_end']!=''){ $totalfilter = $totalfilter + 1; }
                                    ...
                                    @if(count($drivermatch)>0)
                                        @foreach($drivermatch as $item)
                                            @php
                                                ...
                                                if(in_array($item->RecruitmentPosition->recruitment_position_id, explode(',',$filters[0]['position']))){
                                                    $level+=1;
                                                }


                                                for($x=$filters[0]['age_start'];$x<$filters[0]['age_end'];$x++){
                                                    if($x == $item->driver_age){ $level+=1; }
                                                }

                                                $result = '';
                                                foreach($item->DriverLanguage as $lng){
                                                    $result .= $lng->LanguageType->language_type_details.', ';
                                                    if(in_array($lng->LanguageType->language_type_id, explode(',',$filters[0]['language']))){
                                                        $level+=1;
                                                    }
                                                }
                                                ...
                                                foreach($item->WorkHistory as $whtr){
                                                    ...
                                                    if(in_array($whtr->work_history_current_position, explode(',', $filters[0]['history']))){
                                                        $level+=1;
                                                    }
                                                }

                                                if(in_array($item->Districts->districts_id, explode(',', $filters[0]['district']))){ $level+=1; }
                                                if(in_array($item->Districts->Amphures->amphures_id, explode(',', $filters[0]['amphur']))){ $level+=1; }
                                                if(in_array($item->Districts->Provinces->provinces_id, explode(',', $filters[0]['province']))){ $level+=1; }

                                            @endphp
                                            <tr>
                                                <td class="text-center">{{$i++}}</td>
                                                <td class="text-left">{{$item->driver_name_th.' '.$item->driver_lastname_th}}</td>
                                                ...
                                                <td class="text-center">{{$item->Districts->Provinces->provinces_name}}</td>
                                                <td class="text-center">{{rtrim($result,', ')}}</td>
                                                <td class="text-center">{{rtrim($result2,', ')}}</td>
                                                <td class="text-center">@php if($item->driver_smoke=='Y'){ echo 'Yes'; }elseif($item->driver_smoke=='N'){ echo 'No'; }else{ echo '-'; } @endphp</td>
                                                <td class="text-center">
                                                    ...
                                                    <div class="progress" style="height:20px;">
                                                        <div class="progress-bar bg-success text-dark font-12" style="width: <?php echo $levelx; ?>%; height:20px;" role="progressbar"> <?php echo number_format($levelx,2); ?>%</div>
                                                    </div>
                                                </td>
                                                <td class="text-center">
                                                    ...
                                                    <button type="button" class="btn btn-info btn-md btn-view-profile" title="View driver" data-driver_id="{{$item->driver_id}}" data-toggle="modal">
                                                        <i class="icon-eye"></i>
                                                    </button>
                                                </td>
                                            </tr>
