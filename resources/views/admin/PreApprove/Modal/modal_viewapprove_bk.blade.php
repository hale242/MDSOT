                        ...
                        <tr>
                            ...
                            <script>
                                $(document).ready(function(){
                                        var mid = parseInt('<?php echo $QuotationPriceListMain['price_structure_id'] ?>');
                                        var staff = parseInt('<?php echo $CustomerCostId_Val ?>');
                                        var qid = '<?php echo $QuotationPriceListMain['quotation_id']; ?>';
                                        var qmid = '<?php echo $QuotationPriceListMain['quotation_price_list_id']; ?>';
                                        $.post(url_gb+'/admin/PreApprove/GetCost',{typecost:"staff", id:staff, qid:qid, qmid:qmid, _token: $('meta[name="csrf-token"]').attr('content')}, function(data){
                                            // console.log(data);
                                            if(data.length>0){
                                                if(data != undefined){
                                                    if(data[0].quotation_price_list_price != null){
                                                        $('#staff_'+mid+staff).text(data[0].quotation_price_list_price.toLocaleString(undefined, {minimumFractionDigits: 2}));
                                                    }else{
                                                        $('#staff_'+mid+staff).text('-');
                                                    }
                                                }
                                            }else{
                                                    $('#staff_'+mid+staff).text('-');
                            ...
                            <td class="text-center"><span
                                    id="staff_{{$QuotationPriceListMain['price_structure_id']}}{{$CustomerCostId_Val}}"></span>
                            </td>
                            @endforeach
                            ...
                            <script>
                                $(document).ready(function(){
                                        ...
                                        var mid = parseInt('<?php echo $QuotationPriceListMain['price_structure_id'] ?>');
                                        var infid = parseInt('<?php echo $InsuranceFeeCostId_Val ?>');
                                        var qid = '<?php echo $QuotationPriceListMain['quotation_id']; ?>';
                                        var qmid = '<?php echo $QuotationPriceListMain['quotation_price_list_id']; ?>';
                                        $.post(url_gb+'/admin/PreApprove/GetCost',{typecost:"insurance", id:infid, qid:qid, qmid:qmid, _token: $('meta[name="csrf-token"]').attr('content')}, function(data){
                                            // console.log(data);
                                            if(data.length>0){
                                                if(data != undefined){
                                                    if(data[0].quotation_price_list_price != null){
                                                        $('#insurance_'+mid+infid).text(data[0].quotation_price_list_price.toLocaleString(undefined, {minimumFractionDigits: 2}));
                                                    }else{
                                                        $('#insurance_'+mid+infid).text('-');
                                                    }
                                                }
                                            }else{
                                                    $('#insurance_'+mid+infid).text('-');
                            ...
                            <td class="text-center"><span
                                    id="insurance_{{$QuotationPriceListMain['price_structure_id']}}{{$InsuranceFeeCostId_Val}}"></span>
                            </td>
                            @endforeach
                            ...
                            <script>
                                $(document).ready(function(){
                                        var mid = parseInt('<?php echo $QuotationPriceListMain['price_structure_id'] ?>');
                                        var othid = parseInt('<?php echo $OtherCostId_Val ?>');
                                        var qid = '<?php echo $QuotationPriceListMain['quotation_id']; ?>';
                                        var qmid = '<?php echo $QuotationPriceListMain['quotation_price_list_id']; ?>';
                                        $.post(url_gb+'/admin/PreApprove/GetCost',{typecost:"other", id:othid, qid:qid, qmid:qmid, _token: $('meta[name="csrf-token"]').attr('content')}, function(data){
                                            // console.log(data);
                                            if(data.length>0){
                                                if(data != undefined){
                                                    if(data[0].quotation_price_list_price != null){
                                                        $('#othercost_'+mid+othid).text(data[0].quotation_price_list_price.toLocaleString(undefined, {minimumFractionDigits: 2}));
                                                    }else{
                                                        $('#othercost_'+mid+othid).text('-');
                                                    }
                                                }
                                            }else{
                                                    $('#othercost_'+mid+othid).text('-');
                            ...
                            <td class="text-center">
                                <span
                                    id="othercost_{{$QuotationPriceListMain['price_structure_id']}}{{$OtherCostId_Val}}"></span>
                            </td>
                            @endforeach
                            ...
                            <td class="text-right">
                                <b>{{number_format($QuotationPriceListMain['quotation_price_list_price'],2)}}</b></td>
                            <td class="text-right">
                            ...
                            <script>
                                $(document).ready(function(){
                                        var mid = parseInt('<?php echo $QuotationPriceListMain['price_structure_id']; ?>');
                                        var otid = parseInt('<?php echo $StructureOtId["$ic"]; ?>');
                                        var qid = '<?php echo $QuotationPriceListMain['quotation_id']; ?>';
                                        var qmid = '<?php echo $QuotationPriceListMain['quotation_price_list_id']; ?>';

                                        $.post(url_gb+'/admin/PreApprove/GetCost',{typecost:"overtime", id:otid, qid:qid, qmid:qmid, _token: $('meta[name="csrf-token"]').attr('content')}, function(data){
                                            // console.log(data);
                                            if(data.length>0){
                                                if(data != undefined){
                                                    if(data[0].quotation_price_list_ot_price != null){
                                                        $('#otcost_'+mid+otid).text(data[0].quotation_price_list_ot_price.toLocaleString(undefined, {minimumFractionDigits: 2}));
                                                    }else{
                                                        $('#otcost_'+mid+otid).text('-');
                                                    }
                                                }
                                            }else{
                                                    $('#otcost_'+mid+otid).text('-');
                            ...
                            <td class="text-center"><span id="otcost_{{$QuotationPriceListMain['price_structure_id']}}{{$StructureOtId[$ic]}}"></span></td>
                            @php $ic++; @endphp
                            @endforeach
