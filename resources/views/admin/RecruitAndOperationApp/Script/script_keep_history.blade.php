<script>
    $('body').on('click', '.btn-keep-history', function(){
        var id = $(this).data('id');
        var btn = $(this);
        $('#add_keep_driver_interview_id').val(id);
        $("#ModalKeepHistory").modal('show');
    });

    $('body').on('submit', '#FormKeepHistory', function(e) {
        e.preventDefault();
        var form = $(this);
        var id = $('#add_keep_driver_interview_id').val();
    ...
    $('body').on('click' , '.btn-kepp-history-all' , function (){
